...
//   /**
//    * @description Authenticate candidate via form input
//    * @param req Request
//    * @param res Response
//    */
//   candidateAuth = async (req: any, res: any) => {
//     try {
//       const authenticate = await AuthService.verifyUser(req.body);
//       req.session.isLoggedIn = authenticate ? true : false; // Set Logged in session to true
//       req.session.user = authenticate?.user;
//       res.status(200).json({ success: true, data: authenticate });
//     } catch (error) {
...
//   };
  ...
  //   try {
  //     const employeeID = req.session?.user.employeeID;
  //     const user = await AuthService.verifyUser(employeeID);
